...
use super::search_news::{extract_tool_search_news_args, search_news};
...
impl OpenAiClient {
    ...
    async fn tool_response(&self, message: Value, prompt: &str) -> Result<serde_json::Value, AppError> {
        if let Some((keyword, limit)) = extract_tool_search_news_args(&message)? {
            let news = search_news(&keyword, limit).await?;
